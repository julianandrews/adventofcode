...
def connected_components(values, neighbors):
    """Returns a generator over sets of connected nodes.

    Values in the graph must be unique and must implement `__hash__` and
    `__eq__`.

    values    -- an iterable of values representing all nodes in the graph.
    neighbors -- a function from values to an iterable of neighbor values.
    """
...
def toposort(values, neighbors):
    """Returns a list of the provided values in a topological order or None
    if no topological sort exists.
...
def astar(start, is_end, neighbors, edge_weights, heuristic):
    ...
    start        -- the value at which to start the search
    is_end       -- a function which returns true if a value is the destination.
    neighbors    -- a function from values to an iterable of neighbor values.
    edge_weights -- a function which takes two values and returns the cost
                 -- of travel from the first to the second.
    heuristic    -- the A* heuristic function which estimates the remaining
                    distance from a value. This function Must not overestimate
                    distance, and must be consistent (estimates must never increase).
    """
    nodes = [start]
