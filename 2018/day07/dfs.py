...
def dfs(start, neighbors, sort_key=None):
    """Returns a DFSNode generator in depth first order.
    ...
    seen = set([start])
    queue = [DFSNode(start, 0, None, sort_key(start))]
    while queue:
        node = heapq.heappop(queue)
        yield node
        for neighbor in neighbors(node.value):
            if neighbor not in seen:
                neighbor_node = DFSNode(neighbor, node.depth + 1, node, sort_key(neighbor))
